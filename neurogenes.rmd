...
#save gff3 table before continuing in case you want to rerun this chunk
save.gff3.table<-gff3.table

#SUBSAMPLE by desired animals 
filter_target=c("Danior_rerio","Homo_sapiens","Strongylocentrotus_purpuratus")
gff3.table<-filter(gff3.table, animal==filter_target)

#inner join gff3.table x agalma.homologs, by gene to produce the table from which contingency table will be made
...
#plot the clustering
#see tSNE code from http://dpmartin42.github.io/posts/r/cluster-mixed-types
...
#Join all data into a master table

#left join  - true master table has everything
...
#The complete master table has: species_scaffold, start, end, gene, homology_id, catalog_id, animal, cluster_id, species_index
...
NOTE: the number of scaffolds in gff3/fasta for Homo does not match the number in my cluster.master.table. Looks like some scaffolds do not have genes, and I only selected scaffolds w/ genes.
...
analysis.table.comparison<-filter(cluster.master.table,animal==comparison & cluster_id=="2")
each comp scaf
  compare genes
each comp scaf
  compare genes
...
#subset cluster.master.table for the reference or comparison animal; remove any homology_id that is NA - these genes likely arose exclusively in that taxon and is not useful for comparison.
