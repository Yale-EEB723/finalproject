...
NOTE: The group_by() doesn't work if the plyr package is loaded, for some reason.  
...
#filter_target=c("Strongylocentrotus_purpuratus","Homo_sapiens","Danio_rerio")
...
contingency<-contingency[sample(1:nrow(contingency),100,replace=FALSE),sample(1:nrow(contingency),100,replace=FALSE)]
...
#note: animals excluded from cluster analysis will of course not have a cluster_id
...
Debug  
Q: Why does my 100x100 sample never seem to have Strongylo seqs? In contingency.table, there are: 2936 homologs for Hsa, 6209 Dre, 5219 Spu - should have same proportion?  
-not due to order animals are filtered: `filter_target=c("Strongylocentrotus_purpuratus","Homo_sapiens","Danio_rerio")`  
-all three present in `contingency` before sampling 100x100
-BUT in contingeny row names are organized alpha-numerically so Strongylo is always last - > row 100.
-extending sample() to an index that is > row num of Strongylo leads to inclusion of Strongylo rows....! So sample() only takes the first n rows specified (!).
-Changing `contingency<-contingency[sample(100),sample(100)]` to `contingency[sample(1:nrow(contingency),100,replace=FALSE),sample(1:nrow(contingency),100,replace=FALSE)]` leads to inclusion of rows and columns > 100, but almost all rows are Strongylo, no rows are Homo! Random chance? Repeated - yes.
-This is because the number of Strongylo scaffols in contingency before subsetting by far outnumbers Homo/Danio scaffolds:  
From `View(rownames(contingency))` - Homo + Danio ~123, Strongylo ~2253 - 5% of rows are Homo/Danio.  
-This means that in real run through of all data, it will be dominated by whatever genomes have more scaffolds - which are lower quality scaffolds    
-using instead Homo, Danio and Taeniopygia leads to same result as originally. If anything, the cluster 2 big + many small singletons pattern is even more pronounced. But possible now that this pattern only holds for high quality genomes, though it still seems to be a true pattern since null matrix does not match and that used the same sample.  
-But how do we test this for the other genomes with many more lower quality scaffolds? This will always be a problem because these will always outnumber no. scaffolds in high quality genomes.   
-next step: test if this pattern holds if there is a more similar proportion of Strongylo:Homo:Danio scaffs  
...
**DEPRECATED. Replaced by Tree Approach (next chunk)**
...
# To get Agalma homologs even if genetree fails:  
...
# Tree approach to absence analysis  
