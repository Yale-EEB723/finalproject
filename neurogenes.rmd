...
fasta.s <- readDNAStringSet("./data/genomes/tmp/simplified/Hydra_simple.fasta")
...
#Make a master table  
Should double check, but as far as I can tell GFF3 uses all predefined terms - probably can't add anything like a homology_id column without breaking the format.  

```{r MASTA TABLE}
#Parse gff file to extract gene names:  
gff3="data/sample_data/gff3_sample.gff3"

contig.gene.df <- ape::read.gff(file=gff3, GFF3=TRUE) %>% select(.,"seqid","type","start","end","attributes") %>% filter(.,type == "gene")
...
homology_results<-read.csv(file="SQLresults.csv", header=TRUE, sep=",")
